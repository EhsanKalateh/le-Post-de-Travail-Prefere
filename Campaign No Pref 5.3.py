# عالیست ولی ایده آل نیست.
# به دو دلیل. کسی که برخلاف میلش انتخاب میشه، از شانسش برای حضور در سایر بخش ها هم کاسته میشه
# میتونم یکی بنویسم که اگر کسی داوطلب نبود، اون رو اضافه نکنه ولی بهتره خیلی بهش فکر نکنم و به
# درس هام برسم. البته کار سختی نیست احتمالا. فقط اونایی رو بررسی میکنیم که بیشتر از صفر هستن
# مشکل دیگه ای که داره، نیمکت نشین ها هستن که میتونستن درنظر گرفته بشن
# خیلی تا کمال فاصله داره اما خودش خیلیه
import random
random.seed()
All_Shifts=5
Shifts=[[0]]
